    ...
    const fetchNews = async () => {
      try {
        const response = await fetch(
          `https://api.rss2json.com/v1/api.json?rss_url=https://dailypost.ng/politics/feed/&nocache=${Date.now()}`
        );
        ...
        setSiteLogo(data.feed.image || "default-logo.png");
      } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4">
      <h2 className="text-xl lg:text-2xl font-bold text-red-600 mb-6 flex items-center gap-3">
        {siteLogo && <img src={siteLogo} alt="Site Logo" className="w-8 h-8 rounded-full" />}
